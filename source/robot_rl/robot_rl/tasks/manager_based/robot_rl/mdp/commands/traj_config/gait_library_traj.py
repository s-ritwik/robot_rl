...
from robot_rl.tasks.manager_based.robot_rl.terrains.stair_cfg import (
    get_step_height_at_x,
    get_uniform_stair_step_height_from_env,
)
...
class GaitLibraryTrajectoryConfig(BaseTrajectoryConfig):
    ...
    def __init__(
        self,
        gait_library_path: str,
        gait_velocity_ranges: dict[str, tuple] | tuple[float, float, float],
        trajectory_type: str = "end_effector",
    ...
    def _precompute_control_points(self):
        ...
        gait_names = sorted(self.gait_velocity_ranges.keys(), key=lambda name: self.gait_velocity_ranges[name][0])
        ...
        self.left_coeffs_batched = torch.zeros((num_vel, output_dim, degree_plus_1), device=device, dtype=dtype)
        self.right_coeffs_batched = torch.zeros((num_vel, output_dim, degree_plus_1), device=device, dtype=dtype)
    ...
    def get_ref_traj(self, hzd_cmd) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        ref_pos = bezier_deg_batched(
            order=0, tau=tau_expanded, step_dur=step_dur_expanded, control_points=control_points, degree=self.bez_deg
        )  # [num_vel, jt_dim]

        # Use batched Bezier evaluation for velocity (order=1)
        ref_vel = bezier_deg_batched(
            order=1, tau=tau_expanded, step_dur=step_dur_expanded, control_points=control_points, degree=self.bez_deg
        )  # [num_vel, jt_dim]
...
class StairGaitLibraryTrajectoryConfig(GaitLibraryTrajectoryConfig):
    ...
    def __init__(
        self, gait_library_path, gait_height_ranges, trajectory_type="end_effector", config_name="single_support"
    ):
