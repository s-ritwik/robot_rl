...
from robot_rl.tasks.manager_based.robot_rl.mdp.commands.hlip_cmd import (
    _transfer_to_local_frame,
    euler_rates_to_omega,
)
...
class EndEffectorTrajectoryConfig(BaseTrajectoryConfig):
        ...
        for spec in self.constraint_specs:
            ...
            if constraint_type == "com_pos":
                ...
                for i, axis_idx in enumerate(axes):
                    ...
                    self.axis_names.append({"name": metric_name, "index": current_idx + i})
                ...
                for joint_name in joint_names:
                    ...
                    self.axis_names.append({"name": metric_name, "index": current_idx})
    ...
    def get_actual_traj(self, hzd_cmd):
        ...
        y_act = torch.cat(
            [com2stance_local, pelvis_ori, sw2st_foot_pos_local, sw2st_foot_ori, joint_pos.squeeze(-1)], dim=-1
        )

        # concatenate all the velocity values
        dy_act = torch.cat(
            [com_vel_local, pelvis_omega, sw2st_foot_vel_local, sw2st_foot_ang_vel_local, joint_vel.squeeze(-1)], dim=-1
        )
    ...
    def get_actual_traj(self, hzd_cmd):
        ...
        y_act = torch.cat(
            [
                ...
                waist_joint_pos,
                swing_hand_pos,
                swing_hand_ori[:, 2].unsqueeze(-1),
                stance_hand_pos,
                stance_hand_ori[:, 2].unsqueeze(-1),
            ],
            dim=-1,
        )
        ...
        dy_act = torch.cat(
            [
                com_vel_local,
                pelvis_omega,
                sw2st_foot_vel_local,
                sw2st_foot_ang_vel_local,
                waist_joint_vel,
                swing_hand_vel_local,
                swing_hand_ang_vel_local[:, 2].unsqueeze(-1),
                stance_hand_vel_local,
                stance_hand_ang_vel_local[:, 2].unsqueeze(-1),
            ],
            dim=-1,
        )
