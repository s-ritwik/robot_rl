...
class HLIP(torch.nn.Module):
    ...
    def __init__(self, grav: float, z0: float, T_ds: float, T: float, y_nom: float):
        ...
        self.lambda_ = torch.sqrt(
            torch.tensor(grav / z0, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        )
    ...
    def _remap_for_init_stance_state(
        ...
        Y_left = torch.cat(
            [(Y_des_p2[:, 1, 0] - Uy[:, 1]).unsqueeze(-1), Y_des_p2[:, 1, 1].unsqueeze(-1)], dim=-1
        )  # [batch, 2]

        Y_right = torch.cat(
            [(Y_des_p2[:, 0, 0] - Uy[:, 0]).unsqueeze(-1), Y_des_p2[:, 0, 1].unsqueeze(-1)], dim=-1
        )  # [batch, 2]

        X0 = torch.cat([(X_des_p1[:, 0] - Ux).unsqueeze(-1), X_des_p1[:, 1].unsqueeze(-1)], dim=-1)  # [batch, 2]

        return X0, torch.cat([Y_left.unsqueeze(1), Y_right.unsqueeze(1)], dim=1)  # [batch, 2, 2]

    def _compute_desire_com_trajectory(self, cur_time: float, Xdesire: Tensor) -> tuple[Tensor, Tensor]:
    ...
    def _solve_deadbeat_gain(self, A: Tensor, B: Tensor) -> Tensor:
        ...
        A_tmp = torch.stack([
            ...
            torch.tensor([A[1, 1] * B[0] - A[0, 1] * B[1], A[0, 0] * B[1] - A[1, 0] * B[0]]),
        ])
    ...
    def compute_desired_orbit(
        ...
        Y_left = torch.linalg.solve(
            eye_expanded - A_squared, B_term * U_left.unsqueeze(-1) + self.B_s2s * U_right.unsqueeze(-1)
        ).squeeze(
            -1
        )  # [batch, 2]

        Y_right = torch.linalg.solve(
            eye_expanded - A_squared, B_term * U_right.unsqueeze(-1) + self.B_s2s * U_left.unsqueeze(-1)
        ).squeeze(
            -1
        )  # [batch, 2]
    ...
    def compute_orbit(self, T: float, cmd: Tensor) -> None:
