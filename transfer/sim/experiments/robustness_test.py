...
def main():
    ...
    robot_instance = Robot(
        robot_name=config["robot_name"], scene_name=config.get("scene", "basic_scene"), input_function=smooth_ramp
    )
    ...
    if METRIC:
        ...
        if NUM_RUNS > 1:
            axes[0].fill_between(
                added_masses,
                np.maximum(mean_error[:, 0] - std_error[:, 0], 0),
        ...
        if NUM_RUNS > 1:
            axes[1].fill_between(
                added_masses,
                np.maximum(mean_error[:, 1] - std_error[:, 1], 0),
                mean_error[:, 1] + std_error[:, 1],
                color="blue",
                alpha=0.3,
                label="±1 Std Dev",
            )
        ...
        if NUM_RUNS > 1:
            axes[2].fill_between(
                added_masses,
                np.maximum(mean_error[:, 2] - std_error[:, 2], 0),
                mean_error[:, 2] + std_error[:, 2],
                color="blue",
                alpha=0.3,
                label="±1 Std Dev",
            )
        ...
        axes[0].fill_between(
            np.squeeze(time_np[0, :]),
            mean_actual[:, 0] - std_actual[:, 0],
            mean_actual[:, 0] + std_actual[:, 0],
            color="blue",
            alpha=0.3,
            label="±1 Std Dev",
