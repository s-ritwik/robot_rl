...
def main():
    ...
    for i, log in enumerate(logs):
        ...
        if PLOT_MEANS:
            ...
            axes.plot(
                time[ss_idx_start:ss_idx_end],
                np.full(ss_idx_end - ss_idx_start, ss_x_mean),
                linewidth="3",
                color=color,
                linestyle="--",
                label=f"{run_names[i]}_mean",
            )
    ...
    run_handles = [
        Line2D([0], [0], color=run_colors[i], lw=2, linestyle="-", label=run_names[i]) for i in range(len(logs))
    ]
    ...
    axes.legend(
        ...
        fontsize=20,
        columnspacing=0.8,
    )  # Puts all legend entries in one row fontsize = 20,
    # ncol=1, frameon=True)
